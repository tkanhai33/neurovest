...
for (const file of required) {
  if (!existsSync(file)) {
    throw new Error(`Missing frontend brain visualization completion file: ${file}`);
  }
}
...
for (const panel of requiredPanels) {
  if (!shell.includes(panel)) {
    throw new Error(`DashboardShell missing completion panel: ${panel}`);
  }
}

const requiredTerms = {
  "src/features/dashboard/components/BrainVisualizationLayoutPanel.tsx": [
    "Top-down NeuroVest brain visualization foundation",
    "no backend calls",
    "runtime execution",
    "broker calls",
    "AI calls",
...
console.log("PASS: Frontend brain visualization completion certified.");
