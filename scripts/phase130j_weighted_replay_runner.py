...
PHASE = "130J_WEIGHTED_REPLAY_RUNNER"
...
RUN_ID = "WEIGHTED_REPLAY_" + datetime.now(UTC).strftime("%Y%m%d_%H%M%S")
RUN = RUN_ROOT / RUN_ID
RUN.mkdir(parents=True, exist_ok=True)

from backend.app.stacks.strategy_candidate_sandbox.symbol_weights import weighted_symbols

weights = weighted_symbols()

manifest = {
    "phase": PHASE,
    "run_id": RUN_ID,
    "created_at": datetime.now(UTC).isoformat(),
    "mode": "REPLAY_ONLY_WEIGHTED_SYMBOLS",
    "weights": weights,
...
for symbol, weight in sorted(weights.items(), key=lambda x: x[1], reverse=True):
    csv_name = symbol.replace(".TO", "_TO") + ".csv"
    csv_file = FIXTURE_DIR / csv_name
    ...
    for _ in range(repeat):
        weighted_plan.append((symbol, csv_file))
...
overall = {
    ...
    "weighted_plan": [
        {"symbol": symbol, "csv": str(csv_file.relative_to(ROOT))}
        for symbol, csv_file in weighted_plan
    ],
    "symbol_runs": [],
...
for symbol, csv_file in weighted_plan:
    print(f"Running weighted replay {symbol} -> {csv_file}")

    engine = module.ReplayExecutionEngine(RUN)
    summary = engine.execute(csv_file)

    overall["symbol_runs"].append({
        "symbol": symbol,
...
for line in (RUN / "decision_ledger.jsonl").open(encoding="utf-8"):
    if line.strip():
...
(RUN / "130J_weighted_replay_summary.json").write_text(
...
print(json.dumps({
    ...
    "weighted_runs": len(weighted_plan),
