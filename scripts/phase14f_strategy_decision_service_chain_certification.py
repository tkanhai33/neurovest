...
OUT = ROOT / "runtime/certifications/phase14f_strategy_decision_service_chain_certification_latest.json"
OUT.parent.mkdir(parents=True, exist_ok=True)

GRAPH = ROOT / "runtime/repo_memory/strategy_service_chain_graph_v1.json"
...
steps = {
    "patch": run(["python3", "scripts/phase14_strategy_decision_service_chain.py"]),
    "compile_service": run(["python3", "-m", "py_compile", "backend/app/stacks/strategy/strategy_service.py"]),
    "compile_main": run(["python3", "-m", "py_compile", "backend/app/main.py"]),
    "graph_export": run(["python3", "scripts/phase14e_strategy_service_chain_graph_export.py"]),
...
files = {
    ...
    "frontend_service": ROOT / "frontend/services/strategyService.ts",
    "next_route": ROOT / "frontend/app/api/v1/strategy/decision/[symbol]/route.ts",
...
checks = {
    ...
    "main_route_exists": '"/api/v1/strategy/decision/{symbol}"' in texts["main"],
    "frontend_service_exists": files["frontend_service"].exists(),
    "frontend_service_has_getStrategyDecision": "getStrategyDecision" in texts["frontend_service"],
    "next_proxy_exists": files["next_route"].exists(),
    "page_imports_strategy_service": "getStrategyDecision" in texts["page"],
    "strategy_card_rendered": "<StrategyDecisionCard />" in texts["page"],
...
report = {
    "phase": "14F_STRATEGY_DECISION_SERVICE_CHAIN_CERTIFICATION",
