...
OUT = ROOT / "runtime/certifications/phase15f_risk_gate_service_chain_certification_latest.json"
OUT.parent.mkdir(parents=True, exist_ok=True)

GRAPH = ROOT / "runtime/repo_memory/risk_service_chain_graph_v1.json"
...
steps = {
    "patch": run(["python3", "scripts/phase15_risk_gate_service_chain.py"]),
    "compile_service": run(["python3", "-m", "py_compile", "backend/app/stacks/risk/risk_service.py"]),
    "compile_main": run(["python3", "-m", "py_compile", "backend/app/main.py"]),
    "graph_export": run(["python3", "scripts/phase15e_risk_service_chain_graph_export.py"]),
...
files = {
    ...
    "frontend_service": ROOT / "frontend/services/riskService.ts",
    "next_route": ROOT / "frontend/app/api/v1/risk/gate/[symbol]/route.ts",
...
checks = {
    ...
    "main_route_exists": '"/api/v1/risk/gate/{symbol}"' in texts["main"],
    "frontend_service_exists": files["frontend_service"].exists(),
    "frontend_service_has_getRiskGate": "getRiskGate" in texts["frontend_service"],
    "next_proxy_exists": files["next_route"].exists(),
    "page_imports_risk_service": "getRiskGate" in texts["page"],
    "risk_card_rendered": "<RiskGateCard />" in texts["page"],
...
report = {
    "phase": "15F_RISK_GATE_SERVICE_CHAIN_CERTIFICATION",
