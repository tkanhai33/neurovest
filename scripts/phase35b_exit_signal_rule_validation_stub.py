...
VALIDATOR = SANDBOX_DIR / "exit_signal_rule_validator.py"
OUT_DIR = ROOT / "runtime/strategy_candidate_sandbox"
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT = OUT_DIR / "35B_exit_signal_rule_validation_stub_latest.json"

PHASE = "35B_EXIT_SIGNAL_RULE_VALIDATION_STUB"
...
try:
    ...
    from backend.app.stacks.strategy_candidate_sandbox.exit_signal_rule_validator import (
        validate_disabled_exit_rule,
        validate_unsafe_enabled_exit_rule_is_rejected,
    )

    rule = build_disabled_exit_rule_contract("candidate_31e_stub_001", "AAPL")
    safe_validation = validate_disabled_exit_rule(rule)
    unsafe_rejection = validate_unsafe_enabled_exit_rule_is_rejected(rule)
