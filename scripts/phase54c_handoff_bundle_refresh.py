...
OUT = SANDBOX / "54C_handoff_bundle_refresh_latest.json"

PHASE = "54C_HANDOFF_BUNDLE_REFRESH"
...
bundle = {
    ...
    "stage": "Qwen Read-Only Handoff Export Summary Certified",
    ...
    "current_capabilities": [
        ...
        "Qwen read-only handoff export summary",
    ],
    ...
    "next_phase_rule":
        "Create read-only architecture state machine contract only. No writes, execution, or mutation.",
...
try:
    ...
    for path in sorted(SANDBOX.glob("*_latest.json")):
        ...
        if certified:
            bundle["certified_phases"].append(
                phase
            )
        else:
            bundle["failed_or_uncertified_phases"].append(
                phase
            )


    certified_set = set(
        bundle["certified_phases"]
    )

    bundle["file_count"] = len(
        bundle["artifacts"]
    )

    bundle["cert_count"] = len(
        bundle["certified_phases"]
    )


    bundle["checks"]["required_phases_certified"] = (
        REQUIRED_PHASES.issubset(certified_set)
    )

    bundle["checks"]["latest_phase_correct"] = (
        bundle["latest_confirmed_phase"]
        in certified_set
    )

    bundle["checks"]["critical_safety_locks_false"] = all(
        value is False
        for value in bundle["critical_safety_locks"].values()
    )

    bundle["checks"]["has_next_phase"] = bool(
        bundle["next_recommended_phase"]
    )

    bundle["checks"]["no_errors"] = (
        not bundle["errors"]
    )
    ...
    bundle["pipeline_summary"] = {
        ...
        "certified_capability": [
            "Qwen read-only export summary certified",
            "handoff state refreshed",
        ...
        "current_behavior": [
            ...
            "disabled capability reporting only",
            "safe next phase recommendation only",
        ],
    ...
    bundle["certified"] = all(
        bundle["checks"].values()
    )
