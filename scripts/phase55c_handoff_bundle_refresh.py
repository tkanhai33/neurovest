...
OUT = SANDBOX / "55C_handoff_bundle_refresh_latest.json"

PHASE = "55C_HANDOFF_BUNDLE_REFRESH"

SOURCES = [
    SANDBOX / "54C_handoff_bundle_refresh_latest.json",
    SANDBOX / "55B_qwen_read_only_architecture_state_machine_rollup_certification_latest.json",
]
...
try:
    ...
    for source in SOURCES:
        if source.exists():
            loaded[source.name] = json.loads(
                source.read_text(encoding="utf-8")
            )

    latest = loaded.get(
        "55B_qwen_read_only_architecture_state_machine_rollup_certification_latest.json",
        {},
    )

    result["handoff_snapshot"] = {
        "stage": "Qwen Read-Only Architecture State Machine Certified",
        ...
        "next_recommended_phase":
            "56A_QWEN_READ_ONLY_ARCHITECTURE_POLICY_MAP_STUB",
    }

    result["checks"]["source_present"] = len(loaded) == 2
    result["checks"]["state_machine_certified"] = (
        latest.get("certified") is True
    )

    result["checks"]["locks_false"] = all(
        value is False
        for value in result["handoff_snapshot"]
        ["critical_safety_locks"]
        .values()
    )
